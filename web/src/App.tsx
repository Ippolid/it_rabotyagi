...
function Header() {
  ...
  const [active, setActive] = useState<'features' | 'courses' | 'mentors' | null>(null)
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const sections: Array<{ id: 'features'|'courses'|'mentors'; top: number }> = [
        { id: 'features', top: document.getElementById('features')?.offsetTop ?? 0 },
        { id: 'courses', top: document.getElementById('courses')?.offsetTop ?? 0 },
        { id: 'mentors', top: document.getElementById('mentors')?.offsetTop ?? 0 },
      ]
      const current = sections
        .filter(s => s.top <= y + 120)
        .sort((a,b) => b.top - a.top)[0]?.id || null
      setActive(current)
    }
    onScroll()
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/60 bg-white/80 backdrop-blur dark:border-slate-800/60 dark:bg-slate-950/60">
      <div className="container flex h-16 items-center justify-between">
        ...
        <nav className="hidden items-center gap-6 md:flex">
          <a className={`text-sm ${active==='features' ? 'text-slate-900 dark:text-white font-semibold' : 'text-slate-600 dark:text-slate-300'} hover:text-slate-900 dark:hover:text-white`} href="#features">База вопросов</a>
          <a className={`text-sm ${active==='courses' ? 'text-slate-900 dark:text-white font-semibold' : 'text-slate-600 dark:text-slate-300'} hover:text-slate-900 dark:hover:text-white`} href="#courses">Курсы</a>
          <a className={`text-sm ${active==='mentors' ? 'text-slate-900 dark:text-white font-semibold' : 'text-slate-600 dark:text-slate-300'} hover:text-slate-900 dark:hover:text-white`} href="#mentors">Менторы</a>
          <Link className="text-sm text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white" to="/trainer">Тренажер</Link>
        </nav>
        <div className="flex items-center gap-3">
          <AuthBar />
    ...
    <section className="relative overflow-hidden">
      <div className="container py-16 md:py-24">
        ...
        <div className="pointer-events-none relative mt-10 hidden h-[360px] md:block">
          ...
          {/* (removed) bottom pill under the circle */}

          {/* center circle with rocket */}
          <div className="hero-circle glow absolute left-1/2 top-1/2 flex h-56 w-56 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full text-white orb-animate">
    ...
    <section id="mentors" className="relative overflow-hidden">
      <div className="container py-16 md:py-24">
        ...
        {state === 'ready' && (
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {items.map((m) => (
              <div key={m.id} className="glass-card mentor-card p-6 transition-all duration-300">
                <div className="flex items-center gap-4">
                  <div className="avatar" aria-hidden>{getInitials(m.fullName)}</div>
                  <div className="mentor-meta">
                    <div className="mt-0 text-lg font-semibold text-slate-900">{m.fullName}</div>
                    <div className="text-slate-500 text-sm">{m.title}</div>
                  </div>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  {m.skills.map(s => (
                    <span key={s} className="rounded-md px-2.5 py-1 text-xs text-slate-700" style={{background:'linear-gradient(90deg, rgba(255,255,255,0.6), rgba(255,255,255,0.4))', color:'#123'}}>{s}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      listCourses()
        .then(({ items }) => {
          if ((import.meta as any).env?.DEV) {
            // небольшой дебаг, чтобы увидеть реальные данные
            // eslint-disable-next-line no-console
            console.log('Courses loaded:', items)
          }
          setCourses(items as any)
          setState('ready')
        })
        .catch(() => setState('error'))
    )
    ...
    <section id="courses" className="relative overflow-hidden">
      <div className="container py-16 md:py-24">
        ...
        {state === 'loading' && (
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {Array.from({length:3}).map((_,i)=> (
              <article key={i} className="glass-card p-0">
                <div className="skeleton aspect-[16/9] w-full rounded-t-2xl" />
                <div className="p-6">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-3">
          {courses.map((c, idx) => (
            <article key={c.title} className="course-card glass-card p-0 transition-all duration-300">
              <div className={`course-top relative aspect-[16/9] w-full overflow-hidden rounded-t-2xl`} style={{backgroundImage:`url('${imageByIndex(idx)}')`, backgroundSize:'cover', backgroundPosition:'center'}}>
                <div className="flex h-full items-center justify-center text-6xl" style={{backdropFilter:'brightness(.7)'}}>
              ...
              <div className="p-6">
                <h3 className="text-xl font-semibold text-slate-900">{c.title?.trim() ? c.title : `Курс #${c.id}`}</h3>
                <p className="mt-2 text-sm text-slate-600">{c.description?.trim() ? c.description : 'Описание появится скоро'}</p>
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <Link to={`/course/${c.id}`} className="rounded-lg bg-slate-800 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700">
                    Открыть курс
                  </Link>
                </div>
              </div>
  ...
  return (
    <footer className="border-t border-slate-200 py-8 text-sm text-slate-600 dark:border-slate-800 dark:text-slate-300">
      <div className="container flex flex-col items-center justify-between gap-4 md:flex-row">
        <div className="flex items-center gap-3">
          <img src="/logo.jpg" alt="IT-RABOTYAGI" className="footer-logo" />
